...
// SPARS Land Rover Planner Implementation and Benchmark Header

#ifndef SPARS_ROVER_ROVER_H
#define SPARS_ROVER_ROVER_H
...
class SPARSRover : public nav_core::BaseGlobalPlanner {
public:
    SPARSRover();
    SPARSRover(std::string name, costmap_2d::Costmap2DROS* costmap_ros);
...
SPARSRover::SPARSRover(std::string name, costmap_2d::Costmap2DROS* new_costmap_ros) :
        costmap_ros(NULL),
        map_found(false),
        pose_found(false),
        initialized(false),
    ...
    ROS_INFO("Initializing SPARSRover for Turtlebot");
    initializeSubscribers();
    initialize(name, new_costmap_ros);
    ROS_INFO("SPARSRover ready with costmap");
}

SPARSRover::SPARSRover() :
        costmap_ros(NULL),
        map_found(false),
        pose_found(false),
        initialized(false),
    ...
    ROS_INFO("Initializing SPARSRover for Turtlebot");
    initializeSubscribers();
    ROS_INFO("SPARSRover not constructed with costmap");
}

nav_msgs::OccupancyGrid::Ptr SPARSRover::getInflatedMap(){
    return inflated_map;
}

nav_msgs::Path SPARSRover::extractNavPath(ompl::geometric::PathGeometric ompl_path) {
...
PLUGINLIB_EXPORT_CLASS(SPARSRover, nav_core::BaseGlobalPlanner)
